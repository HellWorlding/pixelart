...
namespace PixelColorling
{
    public partial class Coloring : Form
    {
        ...
        private void Coloring_Load(object sender, EventArgs e)
        {
            ...
            rdoBasic.Checked = true; // 기본 모드 선택
        }
        ...
        private void panelCanvas_MouseClick(object sender, MouseEventArgs e)
        {
            ...
            if (rdoBasic.Checked)
                FillCell(y, x);
            else if (rdoSurround.Checked)
                FillSurrounding(y, x, simplifiedSelected);
            else if (rdoAll.Checked)
                FillAllMatching(simplifiedSelected);
        ...
        {
            isFilled[y, x] = true;
            filledColors[y, x] = this.selectedColor;
        }

        // 주변 색칠 (3x3)
        private void FillSurrounding(int y, int x, Color targetColor)
        {
            int h = isFilled.GetLength(0);
            int w = isFilled.GetLength(1);
            ...
            for (int dy = -1; dy <= 1; dy++)
            {
                for (int dx = -1; dx <= 1; dx++)
                {
                    ...
                    {
                        if (SimplifyColor(blockColors[ny, nx], colorCount) == targetColor)
                        {
                            isFilled[ny, nx] = true;
                            filledColors[ny, nx] = this.selectedColor;
                        }
                    }
                }
            }
        ...
        // 선택한 픽셀 색상과 동일한 색상 전부 색칠
        private void FillAllMatching(Color targetColor)
        {
            int h = isFilled.GetLength(0);
            int w = isFilled.GetLength(1);
            ...
            for (int y = 0; y < h; y++)
            {
                for (int x = 0; x < w; x++)
                {
                    if (SimplifyColor(blockColors[y, x], colorCount) == targetColor)
                    {
                        isFilled[y, x] = true;
                        filledColors[y, x] = this.selectedColor;
                    }
